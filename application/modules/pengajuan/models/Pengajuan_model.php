...
class Pengajuan_model extends CI_Model {
	...
	function insert($name,$barang)
	{
		$this->db->trans_begin();
		$this->db->insert('mst_barang', $barang);
		$id  = $this->db->insert_id();
		$this->db->insert('tb_pengajuan',array('tgl_pengajuan'=>$name['tgl_pengajuan'],'prog_kerja'=>$name['prog_kerja'],'kegiatan'=>$name['kegiatan'],'id_barang'=>$id));
	...
	function sispran($tanggal = '',$barang = '')
	{
		...
		$g = $dbs->query("SELECT * FROM buff_ri_modal_v3 WHERE nama_unit = 'Direktorat Sistem dan Teknologi Informasi' AND DATE(tgl_pengajuan_fra) = '{$tanggal}' OR kode_modal LIKE '%$barang%' GROUP BY kode_modal");
		return $g;
	}
